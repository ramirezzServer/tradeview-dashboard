...
function LiveMoverRow({ m }: { m: LiveMover }) {
  ...
  return (
    <div className="flex items-center justify-between py-density-row px-2 group cursor-pointer rounded-lg hover:bg-accent/20 transition-colors">
      <div>
        <p className="text-app-sm font-semibold text-foreground">{m.symbol}</p>
        <p className="text-app-xs text-muted-foreground/40 tabular-nums">
          Vol {m.volume >= 1_000_000 ? `${(m.volume / 1_000_000).toFixed(1)}M` : `${(m.volume / 1_000).toFixed(0)}K`}
        </p>
      </div>
      <div className="flex items-center gap-3">
        <span className="text-app-sm font-bold text-muted-foreground/60 tabular-nums">
          ${m.price.toFixed(2)}
        </span>
        <span className={`flex items-center gap-0.5 text-app-xs font-semibold tabular-nums px-2 py-0.5 rounded-md ${
...
function DemoMoverRow({ m }: { m: MarketMover }) {
  ...
  return (
    <div className="flex items-center justify-between py-density-row px-2 group cursor-pointer rounded-lg hover:bg-accent/20 transition-colors">
      <div>
        <p className="text-app-sm font-semibold text-foreground">{m.symbol}</p>
        <p className="text-app-xs text-muted-foreground/40">{m.name}</p>
      </div>
      <div className="flex items-center gap-3">
        <span className="text-app-sm font-bold text-muted-foreground/40 tabular-nums">
          ${m.price.toFixed(2)}
        </span>
        <span className={`flex items-center gap-0.5 text-app-xs font-semibold tabular-nums px-2 py-0.5 rounded-md ${
          positive ? 'text-bull bg-bull/8' : 'text-bear bg-bear/8'
  ...
  return (
    <div className="glass-card rounded-xl overflow-hidden">
      ...
      {(showQuotaNotice || showConfigNotice) && (
        <div className="mx-4 mb-2 flex items-start gap-density-row p-density-card rounded-lg bg-chart-accent/8 border border-chart-accent/15">
          <AlertCircle className="h-3 w-3 text-chart-accent/60 mt-0.5 shrink-0" />
          <p className="text-app-xs text-muted-foreground/50 leading-snug">
            {showQuotaNotice
      ...
      <div className="px-density-card pb-3">
        <Tabs defaultValue="gainers">
          <TabsList className="h-7 w-full bg-secondary/30 border border-border/20 rounded-lg p-0.5">
            <TabsTrigger value="gainers"  className="text-app-xs flex-1 rounded-md data-[state=active]:bg-primary/12 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_8px_-2px_hsl(var(--primary)/0.15)]">Gainers</TabsTrigger>
            <TabsTrigger value="losers"   className="text-app-xs flex-1 rounded-md data-[state=active]:bg-primary/12 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_8px_-2px_hsl(var(--primary)/0.15)]">Losers</TabsTrigger>
            <TabsTrigger value="trending" className="text-app-xs flex-1 rounded-md data-[state=active]:bg-primary/12 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_8px_-2px_hsl(var(--primary)/0.15)]">Active</TabsTrigger>
          </TabsList>
