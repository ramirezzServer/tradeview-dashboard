...
const revenueChart = [
  { q: 'Q1 24', rev: 82.5 }, { q: 'Q2 24', rev: 78.4 }, { q: 'Q3 24', rev: 89.5 },
  { q: 'Q4 24', rev: 111.4 }, { q: 'Q1 25', rev: 90.8 }, { q: 'Q2 25', rev: 85.8 },
  { q: 'Q3 25', rev: 94.9 }, { q: 'Q4 25', rev: 119.6 },
];
...
const FinancialSnapshot = () => {
  const { data: profileData, loading, isLive } = useFinnhubProfile('AAPL');

  const keyMetrics = isLive && profileData.profile
    ? [
        ...
        { label: 'Market Cap', value: profileData.profile.marketCapitalization > 1000 ? `$${(profileData.profile.marketCapitalization / 1000).toFixed(2)}T` : `$${profileData.profile.marketCapitalization.toFixed(0)}B`, change: '+18.6%', positive: true },
        ...
        { label: 'Website', value: 'apple.com', change: 'IR', positive: true, neutral: true },
      ]
    : mockKeyMetrics;

  const valuationData = mockValuationMetrics.map(m => ({
    label: m.label,
    value: isLive && profileData.metrics[m.key] !== undefined && profileData.metrics[m.key] !== null
    ...
    <DashboardLayout title="Financial Snapshot">
      <div className="p-4 lg:p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {loading ? (
            [...Array(6)].map((_, i) => (
              <Skeleton key={i} className="h-20 rounded-xl bg-secondary/20" />
            ))
          ) : (
            keyMetrics.map((f, i) => (
